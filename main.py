...
class Pipeline():
    def __init__(self, oral_type):
        ...
        self.label_df = pd.read_csv(f'./cropped_dataset/val/{oral_type}/csv/metadata.csv')
        self.label_df.set_index('img_dir', inplace=True)
    ...
    def run_pipeline(self, results, cls_model, device):
        ...
        for result in results: # result: val 구강 이미지
            ...
            if result.masks is not None:
                ...
                speed.append((end_time - start_time).total_seconds())
...
if __name__ == '__main__':
    ...
    start_ready_speed = datetime.now()
    pipe.set_seg_model(f'./runs/segment/{oral_type}/weights/best.pt')
    end_ready_speed = datetime.now()
    print(f'ready time: {end_ready_speed - start_ready_speed} seconds')
    ...
    print(f'Average processing time per image: {np.mean(speed)} seconds')